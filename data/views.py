...
def attendance(request, stud_id):
    ...
    for ass in ass_list:
        try:
            ...
            a.save()
        att_list.append(a)
    return render(request, 'data/attendance.html', {'att_list': att_list})
...
def attendance_detail(request, stud_id, course_id):
    ...
    return render(request, 'data/att_detail.html', {'att_list': att_list, 'cr': cr})
...
def t_clas(request, teacher_id, choice):
    ...
    return render(request, 'data/t_clas.html', {'teacher1': teacher1, 'choice': choice})
...
def t_student(request, assign_id):
    ...
    for stud in ass.class_id.student_set.all():
        try:
            ...
            a.save()
        att_list.append(a)
    return render(request, 'data/t_students.html', {'att_list': att_list})
...
def t_class_date(request, assign_id):
    ...
    return render(request, 'data/t_class_date.html', {'att_list': att_list})
...
def t_attendance(request, ass_c_id):
    ...
    return render(request, 'data/t_attendance.html', context)
...
def edit_att(request, ass_c_id):
    ...
    return render(request, 'data/t_edit_att.html', context)
...
def t_attendance_detail(request, stud_id, course_id):
    ...
    cr = get_object_or_404(Course, id=course_id)
    att_list = Attendance.objects.filter(course=cr, student=stud).order_by('date')
    return render(request, 'data/t_att_detail.html', {'att_list': att_list, 'cr': cr})
...
def change_att(request, att_id):
    ...
    a.save()
    return HttpResponseRedirect(reverse('t_attendance_detail', args=(a.student.roll_no, a.course_id)))
...
def timetable(request, class_id):
    ...
    matrix = [['' for i in range(11)] for j in range(5)]
...
def t_timetable(request, teacher_id):
    ...
    class_matrix = [[True for i in range(11)] for j in range(5)]
    for i, d in enumerate(DAYS_OF_WEEK):
        t = 0
        for j in range(11):
            if j == 0:
...
def free_teachers(request, asst_id):
    ...
    return render(request, 'data/free_teachers.html', {'ft_list': ft_list})
...
def marks_list(request, stud_id):
    ...
    for ass in ass_list:
        try:
            ...
            sc.marks_set.create(type='M', name='Mid Semester Exams')
            sc.marks_set.create(type='S', name='End Semester Exam')
        sc_list.append(sc)

    return render(request, 'data/marks_list.html', {'sc_list': sc_list})
...
def t_marks_list(request, assign_id):
    ...
    return render(request, 'data/t_marks_list.html', {'m_list': m_list})
...
def t_marks_entry(request, marks_c_id):
    ...
    return render(request, 'data/t_marks_entry.html', context)
...
def edit_marks(request, marks_c_id):
    ...
    return render(request, 'data/edit_marks.html', context)
...
def student_marks(request, assign_id):
    ...
    return render(request, 'data/t_student_marks.html', {'sc_list': sc_list})
...
def add_teacher(request):
    ...
    return render(request, 'data/add_teacher.html', context)
...
def add_student(request):
    ...
    if request.method == 'POST':
        ...
        # USERNAME: firstname + underscore + last 3 digits of roll_no
        # PASSWORD: firstname + underscore + year of birth(YYYY)
        user = User.objects.create_user(
            username=name.split(" ")[0].lower() + '_' + request.POST['roll_no'][-3:],
            password=name.split(" ")[0].lower() + '_' + dob.replace("-","")[:4]
        )
        user.save()
    ...
    return render(request, 'data/add_student.html', context)